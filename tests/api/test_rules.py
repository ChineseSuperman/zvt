...
def test_is_trading_date():
    assert is_trading_date(security_type=EntityType.stock, exchange=None, timestamp='2019-06-28')
    assert not is_trading_date(security_type=EntityType.stock, exchange=None, timestamp='2019-06-22')
    assert not is_trading_date(security_type=EntityType.stock, exchange=None, timestamp='2019-06-23')


def test_is_open_close_time():
    timestamps = iterate_timestamps(security_type=EntityType.coin, exchange='binance',
                                    level=IntervalLevel.LEVEL_1MIN, start_timestamp='2019-05-01',
                                    end_timestamp='2019-05-01')
    assert is_open_time(security_type=EntityType.coin, exchange='binance', timestamp=timestamps[0])
    assert is_close_time(security_type=EntityType.coin, exchange='binance', timestamp=timestamps[-1])
    ...
    for timestamp in timestamps:
        if is_open_time(security_type=EntityType.coin, exchange='binance', timestamp=timestamp):
            open.append(timestamp)
        if is_close_time(security_type=EntityType.coin, exchange='binance', timestamp=timestamp):
...
def test_iterate_stock_timestamps():
    timestamps = iterate_timestamps(security_type=EntityType.stock, exchange='sh',
                                    level=IntervalLevel.LEVEL_1HOUR, start_timestamp='2019-05-01',
                                    end_timestamp='2019-05-02')
...
def test_is_in_finished_timestamps():
    assert is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01',
                                     level=IntervalLevel.LEVEL_1DAY)
    assert is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 15:00',
                                     level=IntervalLevel.LEVEL_1DAY)
    assert not is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 14:00',
                                         level=IntervalLevel.LEVEL_1DAY)

    assert is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 09:31',
                                     level=IntervalLevel.LEVEL_1MIN)
    assert not is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 09:30',
                                         level=IntervalLevel.LEVEL_1MIN)

    assert not is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 08:30',
                                         level=IntervalLevel.LEVEL_1MIN)

    assert is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 11:30',
                                     level=IntervalLevel.LEVEL_1MIN)
    assert is_in_finished_timestamps(security_type=EntityType.stock, exchange='sh', timestamp='1999-01-01 11:30',
