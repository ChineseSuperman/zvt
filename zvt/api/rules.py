...
from zvt.domain import EntityType, IntervalLevel, to_pd_timestamp
...
def get_trading_intervals(entity_type, exchange):
    ...
    if entity_type == EntityType.stock and exchange in ('sh', 'sz'):
...
for level in IntervalLevel:
    if level < IntervalLevel.LEVEL_1DAY:
        china_stock_level_map_finished_timestamps[level.value] = generate_finished_timestamps(
            entity_type=EntityType.stock, exchange='sh', level=level)
...
def get_trading_meta(entity_id=None, entity_type=None, exchange=None):
    ...
    if entity_type == EntityType.future:
        return {
            'trading_t': 0,
            'could_short': True
        }
...
if __name__ == '__main__':
    # print(is_in_finished_timestamps(entity_type='stock', exchange='sh', level=TradingLevel.LEVEL_1HOUR,
